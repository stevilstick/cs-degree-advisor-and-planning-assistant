...
class ApiV1SemestersTest < ActionDispatch::IntegrationTest
  ...
  test "creates a semester" do
    post "/api/v1/semesters/", { year_id: @year.id, semester_definition_id: 1 }, { "Accept" => "application/json" }
  ...
  test "should update a semester" do
    semester = FactoryGirl.create :semester, year_id: 1, semester_definition_id: 1
    put "/api/v1/semesters/#{semester['id']}", { semester_definition_id: 2 }, { "Accept" => "application/json" }

    assert_equal(200, response.status)

    body = JSON.parse(response.body)
    assert_equal(1, body['semester']['semester_definition_id'])
  end
  ...
  test "should return a single semester" do
    semester = FactoryGirl.create :semester, year_id: 1, semester_definition_id: @winterim.id
    get "/api/v1/semesters/#{semester['id']}", {}, { "Accept" => "application/json" }
    body = JSON.parse(response.body)

    assert_equal(200, response.status, "has successful response code")
    assert_equal(@winterim.id, body['semester']['semester_definition_id'], "Has the correct name")
  ...
  test "should return all the semesters" do
    ...
    FactoryGirl.create :semester, year_id: @year2.id, semester_definition_id: @winterim.id
    FactoryGirl.create :semester, year_id: @year1.id, semester_definition_id: @spring.id
    FactoryGirl.create :semester, year_id: @year3.id, semester_definition_id: @summer.id
    ...
    assert_not_nil(body['semesters'].first['semester_definition_id'])
  end
  ...
  test "should delete a semester" do
    semester1 = FactoryGirl.create :semester, year_id: @year.id, semester_definition_id: @winterim.id
