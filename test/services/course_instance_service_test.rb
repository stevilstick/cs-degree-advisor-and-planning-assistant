...
class CourseInstancesServicesTest < ActionController::TestCase
  ...
  test "should return more then one course instance's with semester_id equal to 1" do
    context = {semester_id:@semester.id}
    result = CourseInstanceService.new.find(context)
  ...
  test "should return more than one course instance with year_id equal to 1" do
